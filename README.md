...
The runtime is solo by design: once Eli is trained, you load `model.pt`
+ the active partner's LoRA + tokenizer, and that *is* Eli. No Groq, no
OpenAI, no Anthropic, no remote service of any kind. This is the load-
bearing claim of the project — "the memory is in the weights, not in a
database, and the response is generated by the weights, not by an LLM
proxying the substrate state."
...
Or the full interactive runtime (defaults to SOLO mode — no LLM):

```bash
py -m substrate_self.converse           # uses ~/.substrate-self/model.pt
```

If you have NOT yet trained a model from your own corpus, `converse`
falls back to a bootstrap mode where Groq (Llama 3.3 70B, free tier)
acts as a *training-wheels voice* while you collect dialogue for the
teacher pipeline — the LLM is **not** Eli, it's the bootstrap helper
described in *LLM as teacher, model as runtime*. To require solo mode
strictly, pass `--solo` (errors out if no trained model is found):

```bash
py -m substrate_self.converse --solo
```
...
entity, not an LLM with a memory plugin.
